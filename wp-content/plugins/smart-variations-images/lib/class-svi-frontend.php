...
class woocommerce_svi_frontend {
    ...
    function load_scripts() {
        ...
        /* if ($this->woosvi_options['lightbox']) {
          ...
          wp_enqueue_script('svi_lightgallery', '//cdnjs.cloudflare.com/ajax/libs/lightgallery/1.2.21/js/lightgallery.min.js', $loads, true);
          array_push($loads, 'svi_lightgallery');
          wp_enqueue_script('svi_lightgallerythumb', '//cdnjs.cloudflare.com/ajax/libs/lightgallery/1.2.21/js/lg-thumbnail.min.js', $loads, true);
          wp_enqueue_style('svi_lightgallerycss', '//cdnjs.cloudflare.com/ajax/libs/lightgallery/1.2.21/css/lightgallery.min.css', null, null);
          array_push($loads, 'svi_lightgallerythumb');
          } */
        if ($this->woosvi_options['lightbox']) {
            # prettyPhoto
    ...
    public function imgArray($id) {
        ...
        $alt = $title = get_post($id)->post_excerpt;

        $data_single = wp_get_attachment_image($id, apply_filters('single_product_large_thumbnail_size', 'shop_single'), 0, $attr = array(
            'title' => $title,
            'alt' => $alt,
        ));

        $filter = 'shop_thumbnail';
        ...
        return array(
            ...
            'img' => wp_get_attachment_image($id, apply_filters('single_product_large_thumbnail_size', 'shop_single'), 0, $attr = array(
                'title' => $title,
                'alt' => $alt,
                'data-woosvi' => $woosvi_slug
            )),
            'img_thumb' => wp_get_attachment_image($id, $filter, 0, $attr = array(
                'title' => $title,
                'alt' => $alt,
                'data-svisingle' => str_replace('"', "'", $data_single),
                'data-woosvi' => $woosvi_slug
            )),
